...
export default function LandingPage() {
    ...
    return (
        <div className="bg-white min-h-screen text-slate-body font-light">
            {/* Navigation */}
            <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-slate-border">
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3 md:gap-6">
                        <Link href="/login" className="text-sm font-normal text-slate-body hover:text-navy-deep transition-colors whitespace-nowrap">Sign in</Link>
                        <Link href="/signup">
            ...
            <section className="pt-40 pb-0 overflow-hidden">
                <div className="max-w-7xl mx-auto px-6 flex flex-col gap-16 items-center text-center">
                    <div className="max-w-3xl flex flex-col items-center">
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-purple/5 border border-brand-purple/10 rounded-full text-brand-purple text-[10px] font-normal uppercase tracking-widest mb-8">
                            <Zap className="w-3 h-3" />
                            Join early access
                        </div>
                        <h1 className="text-5xl md:text-6xl font-light text-navy-deep leading-tight-display tracking-tight-large mb-8">
                            The operating system for <span className="text-brand-purple font-normal">loan automation.</span>
                        </h1>
                        <p className="text-lg text-slate-body leading-relaxed mb-10 max-w-xl">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 justify-center">
                            <Link href="/signup">
                                <Button size="lg" className="h-12 px-8 text-md shadow-stripe-ambient">
                                    Get Started
                                    <ArrowRight className="w-5 h-5 ml-2" />
                                </Button>
                            </Link>
                            <Button variant="ghost" size="lg" className="h-12 px-8 text-md text-slate-body">
                    ...
                    <div className="relative z-10 self-end max-w-5xl mx-auto">
                        ...
                        <img
                            ...
                            className="w-full h-auto scale-100 origin-bottom animate-in fade-in slide-in-from-bottom-8 duration-1000"
                        />
                        {/* Decorative elements */}
            ...
            {/* Benefits */}
            <section className="py-24 bg-slate-50 border-y border-slate-border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-left">
                        {[
                            ...
                            <div key={i} className="space-y-4">
                                <div className="w-12 h-12 bg-white border border-slate-border rounded-[5px] flex items-center justify-center text-brand-purple mx-auto md:mx-0 shadow-stripe transition-transform hover:scale-105">
                                    <benefit.icon className="w-6 h-6" />
                                </div>
                                <h3 className="text-xl font-normal text-navy-deep tracking-tight">{benefit.title}</h3>
                                <p className="text-slate-body leading-relaxed text-sm font-light">{benefit.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="py-32 bg-white">
                <div className="max-w-3xl mx-auto px-6">
                    <div className="text-center mb-16">
                        <h2 className="text-3xl md:text-4xl font-light text-navy-deep mb-4 tracking-tight-large">Frequently Asked Questions</h2>
                        <p className="text-slate-body font-light">Everything you need to know about the Lydraflow platform.</p>
                    </div>
                    <div className="space-y-4">
                        {[
                            ...
                            <div key={i} className="border border-slate-border rounded-[5px] overflow-hidden shadow-stripe-ambient">
                                <button
                                    onClick={() => setOpenIndex(openIndex === i ? null : i)}
                                    className="w-full px-8 py-6 flex items-center justify-between text-left hover:bg-slate-50 transition-colors"
                                >
                                    <span className="text-lg font-normal text-navy-deep tracking-tight">{faq.q}</span>
                                ...
                                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openIndex === i ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="px-8 pb-6 text-slate-body text-sm leading-relaxed border-t border-slate-border pt-4 font-light">
                                        {faq.a}
                                    </div>
                                </div>
                            </div>
            ...
            <section className="py-32 bg-white relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
                    <h2 className="text-4xl md:text-5xl font-light text-navy-deep mb-8 max-w-2xl mx-auto leading-tight-display tracking-tight-large">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                        <Link href="/signup">
                            <Button size="lg" className="h-14 px-12 text-lg shadow-stripe-ambient">Get Started Now</Button>
                        </Link>
                        <Button variant="ghost" size="lg" className="h-14 px-10 text-lg text-slate-body font-normal">Contact Sales</Button>
                    </div>
                </div>
            </section >

            {/* Footer */}
            <footer className="py-12 border-t border-slate-border bg-white">
                <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="flex items-center">
                        <img src="/images/Lydra4.png" alt="Lydraflow" className="w-18 h-18 object-contain" />
                    </div>
                    <div className="flex gap-8 text-sm text-slate-body">
                        <Link href="#" className="hover:text-navy-deep transition-colors font-normal">Documentation</Link>
                        <Link href="#" className="hover:text-navy-deep transition-colors font-normal">Privacy Policy</Link>
                        <Link href="#" className="hover:text-navy-deep transition-colors font-normal">Terms of Service</Link>
                    </div>
                    <p className="text-xs text-slate-body font-normal opacity-60">© 2026 Lydraflow Technologies Ltd.</p>
                </div>
            </footer>
        </div >
    );
